...
export default function Home() {
  ...
  return <main>
    ...
    <section className="work-section" id="trabajos"><div className="section-heading"><div><p className="eyebrow">Portfolio completo</p><h2>Trabajos<br /><em>audiovisuales.</em></h2></div><p className="section-count">{pages.length} áreas</p></div><div className="category-nav">{pages.map((page) => <a href={`#${page.section}`} key={page.section}>{sectionNames[page.section]} <span>↘</span></a>)}</div>{pages.map((page) => <section className="portfolio-category" id={page.section} key={page.section}><div className="category-heading"><span className="category-number">0{pages.indexOf(page) + 1}</span><div><h3>{sectionNames[page.section]}</h3><p>{sectionDescriptions[page.section]}</p><small>{cleanImages(page).length} fotografías · {videosFor(page).length} videos</small></div></div><Gallery page={page} /><div className="video-heading"><p className="eyebrow">Piezas audiovisuales</p><span>Reproducibles directamente en la página</span></div><VideoGrid page={page} /></section>)}</section>
