...
const cleanImages = (page: Page) => page.images.filter((image) => !image.source.includes("sociallinks"));
...
function Gallery({ page }: { page: Page }) {
  ...
  return <div className="full-gallery">{images.map((image, index) => <div className="gallery-item" key={image.local}><Image src={image.local} alt={`${sectionNames[page.section]} - imagen ${index + 1}`} fill sizes="(max-width: 700px) 92vw, (max-width: 1100px) 45vw, 30vw" /></div>)}</div>;
}
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="inicio"><p className="eyebrow">Directora · Asistente de dirección · Técnica audiovisual</p><h1>Micaela<br /><em>Wallace</em></h1><div className="hero-bottom"><p>Historias, cuerpos y espacios<br />en movimiento.</p><a className="scroll-cue" href="#trabajos" aria-label="Ver portfolio">↓<span>Explorar</span></a><p className="hero-index">{String(totalImages).padStart(2, "0")} fotos<br />{String(totalVideos).padStart(2, "0")} videos</p></div></section>
    <section className="work-section" id="trabajos"><div className="section-heading"><div><p className="eyebrow">Portfolio completo</p><h2>Una práctica<br /><em>en movimiento.</em></h2></div><p className="section-count">{pages.length} áreas</p></div><div className="category-nav">{pages.map((page) => <a href={`#${page.section}`} key={page.section}>{sectionNames[page.section]} <span>↘</span></a>)}</div>{pages.map((page) => <section className="portfolio-category" id={page.section} key={page.section}><div className="category-heading"><span className="category-number">0{pages.indexOf(page) + 1}</span><div><h3>{sectionNames[page.section]}</h3><p>{cleanImages(page).length} fotografías · {videosFor(page).length} videos</p></div><a href={page.url} target="_blank" rel="noreferrer">Ver fuente ↗</a></div><Gallery page={page} /><div className="video-heading"><p className="eyebrow">Piezas audiovisuales</p><span>Todos los trabajos de esta sección</span></div><VideoGrid page={page} /></section>)}</section>
